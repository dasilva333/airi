...
function buildPreviewItems(entry: FlowEntry): PreviewItem[] {
  ...
  if (entry.type.startsWith('spark:')) {
    if (payload?.headline) items.push({ label: 'Headline', value: formatPreviewValue(payload.headline) })
    if (payload?.state) items.push({ label: 'State', value: formatPreviewValue(payload.state) })
    if (payload?.intent) items.push({ label: 'Intent', value: formatPreviewValue(payload.intent) })
  }
...
function buildSparkCommandPreview(command: WebSocketEvents['spark:command']): PreviewItem[] {
  const items: PreviewItem[] = []
  if (command.intent) items.push({ label: 'Intent', value: formatPreviewValue(command.intent) })
  if (command.priority) items.push({ label: 'Priority', value: formatPreviewValue(command.priority) })
  if (command.interrupt !== undefined) items.push({ label: 'Interrupt', value: formatPreviewValue(command.interrupt) })
  ...
  if (command.ack) items.push({ label: 'Ack', value: formatPreviewValue(command.ack) })
  if (command.guidance !== undefined) items.push({ label: 'Guidance', value: formatPreviewValue(command.guidance) })
  if (command.contexts !== undefined) items.push({ label: 'Contexts', value: formatPreviewValue(command.contexts) })
  return items
}
