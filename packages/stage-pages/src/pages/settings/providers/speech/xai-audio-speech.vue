...
const providerId = 'xai-audio-speech'
const defaultModel = 'grok-2-tts'
...
// Get available voices from speech store
...
onMounted(async () => {
  ...
  // Load voices - xAI provides 6 voices: Ara, Rex, Sal, Eve, Una, Leo
  await speechStore.loadVoicesForProvider(providerId)
})

// Generate speech with xAI-specific parameters
async function handleGenerateSpeech(input: string, voiceId: string, _useSSML: boolean) {
  ...
  // Use the reactive model computed property
...
<template>
  ...
  >
    <!-- Voice settings specific to xAI -->
    ...
    <template #playground>
      <SpeechPlayground
        ...
        default-text="[excited] Hello there! Welcome to xAI Grok Speech! [whispers] This is a secret... [laughs] Just kidding! [yells] Let's have some fun!"
      />
    </template>
  </SpeechProviderSettings>
</template>
