...
export function useVersionedLocalStorage<T>(
  ...
  const rawValue = useLocalStorage<Versioned<T>>(
    key,
        ...
        if ('version' in rawValue.value && rawValue.value.version != null) {
          if (options?.satisfiesVersionBy != null && !options.satisfiesVersionBy(rawValue.value.version)) {
            if (options.onVersionMismatch != null) {
              ...
              console.warn(
                `version ${rawValue.value.version} doesn't satisfy the version ${options.defaultVersion} for key ${key}, will reset the value to default value ${toValue(initialValue)}`,
              )
              rawValue.value = { data: toValue(initialValue), version: defaultVersion }
              data.value = toValue(initialValue)
            }
