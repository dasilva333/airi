...
function getTextFromContentParts(parts: unknown[]): string {
  return parts
    .reduce<string[]>((texts, part) => {
      if (!isTextPart(part)) return texts
...
export function getChatHistoryItemCopyText(message: ChatHistoryItem): string {
  if (message.role === 'error') return message.content
  ...
  if (message.role === 'assistant') {
    ...
    if (typeof message.content === 'string') return stripMarkers(message.content)
...
export function getChatHistoryItemKey(message: ChatHistoryItem | undefined, index: number): string | number {
  ...
  if (message.createdAt != null) return `${message.role}:${message.createdAt}:${index}`
