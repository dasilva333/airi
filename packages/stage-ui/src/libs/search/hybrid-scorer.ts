...
function normalizeWeights(weightVector: number, weightKeyword: number) {
  ...
  if (total <= 0) return { weightKeyword: 0.5, weightVector: 0.5 }
...
export function detectQueryProfile(query: string): QueryProfile {
  ...
  if (normalized.endsWith('?') || /^(who|what|when|where|why|how)\b/.test(normalized)) {
    return 'question'
  }

  if (tokenCount >= 14) return 'longform'
...
function filterVectorNoise(vectorScores: number[], config: ScorerConfig) {
  if (!vectorScores.length) return false

  const best = Math.max(...vectorScores)
  if (best < config.minVectorSimilarity) return true
...
export function scoreHybridResults(
  ...
  const results = documents
    .map((document) => {
      ...
      const signalScore =
        effectiveConfig.weightVector * (dropVectorSignal ? 0 : rrfVectorScore) +
