...
/**
 * Aggregates token usage across all completed stream steps.
 *
 * NOTICE: We deliberately sum each step's FINAL usage object instead of using
 * `result.totalUsage` from @xsai/stream-text. That library's `pushUsage` adds the
 * `usage` field of EVERY SSE chunk that carries one (node_modules/@xsai/stream-text
 * dist/index.js), so gateways streaming cumulative usage in every chunk (Groq-style)
 * make `totalUsage` grow quadratically. The last usage pushed within a step is the
 * provider's cumulative final — summing those per step stays correct for
 * single-chunk providers, per-chunk cumulative providers, and multi-step tool loops.
 */
function aggregateStepUsage(steps: Array<{ usage?: unknown }> | undefined | null) {
  let promptTokens = 0
