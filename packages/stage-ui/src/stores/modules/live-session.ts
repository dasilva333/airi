...
import type { LatestTurnUsage } from './gemini-live-seams'
...
export { type LatestTurnUsage, mapAiriToolToGemini, sanitizeTokenCount }

const MODEL = GEMINI_LIVE_MODEL
const LIVE_WS_BASE = GEMINI_LIVE_WS_BASE
...
export const useLiveSessionStore = defineStore('live-session', () => {
  ...
  async function executeToolCall(
    ...
    const rateCheck = checkToolRateLimit(toolCallCounter, MAX_TOOL_CALLS_PER_TURN, { callId, name })
    if (!rateCheck.allowed) {
      debug(`[LiveSession] ⚠️ Rate limit reached (${MAX_TOOL_CALLS_PER_TURN} calls). Skipping tool: ${name}`)
      if (ws.readyState === WebSocket.OPEN && rateCheck.rateLimitPayload) {
        ws.send(JSON.stringify(rateCheck.rateLimitPayload))
      }
      return
    }
    ...
    try {
      ...
      if (!matchedTool) {
        ...
        if (ws.readyState === WebSocket.OPEN) {
          ws.send(JSON.stringify(formatToolResponsePayload(callId, name, new Error(`Tool "${name}" not found in registry.`))))
        }
        return
      }
      ...
      if (ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify(formatToolResponsePayload(callId, name, result)))
      }
      ...
      if (ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify(formatToolResponsePayload(callId, name, err instanceof Error ? err : new Error(String(err)))))
      }
    }
  }
  ...
  function start() {
    ...
    ws.onopen = async () => {
      ...
      const setupMessage = buildLiveSetupMessage({
        ...
        systemPrompt: airiCard.systemPrompt || 'You are an AI assistant.',
        resolvedTools: resolvedToolRegistry,
        isGroundingEnabled: isGroundingEnabled.value,
      })

      debug('[LiveSession] Sending setup with mandatory AUDIO modality:', JSON.stringify(setupMessage, null, 2))
  ...
  function playPcmChunk(base64Data: string) {
    ...
    try {
      ...
      const float32 = decodePcm16Chunk(base64Data)
      debug(`[LiveSession] Decoded ${float32.length} samples of PCM data`)

      // Create AudioBuffer at Gemini's native 24kHz sample rate
      const audioBuffer = ctx.createBuffer(1, float32.length, GEMINI_LIVE_SAMPLE_RATE)
  ...
  function recordInferenceUsage(usage: number | object | null | undefined) {
    ...
    const payload = extractInferenceUsage(usage)
    ...
    if (payload) {
      ...
      inferenceTokens.value = sanitizeTokenCount(inferenceTokens.value) + payload.total
      inferencePromptTokens.value = sanitizeTokenCount(inferencePromptTokens.value) + payload.prompt
      inferenceCompletionTokens.value = sanitizeTokenCount(inferenceCompletionTokens.value) + payload.completion
      debug(`[LiveSession] Inference usage recorded: +${payload.total}. New total inference: ${inferenceTokens.value}`)
    }
    else {
