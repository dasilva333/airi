...
export const AiriPacingSchema = object({
  ...
  maxFillerDurationMs: optional(pipe(number(), integer(), minValue(400), maxValue(6000))),
  ...
  maxFillerSynthesisBudgetMs: optional(pipe(number(), integer(), minValue(100), maxValue(6000))),
  maxSynthesisBudgetMs: optional(pipe(number(), integer(), minValue(100), maxValue(6000))),
  pacingProfile: optional(union([
    literal('snappy'),
    literal('balanced'),
    literal('deep_cot'),
    literal('custom'),
