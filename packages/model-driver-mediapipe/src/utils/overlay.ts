...
function drawFace(drawing: DrawingUtils, face: FaceState) {
  if (!face.landmarks2d?.length) return
...
function drawPose(drawing: DrawingUtils, pose: PoseState) {
  if (!pose.landmarks2d?.length) return

  const colors = paletteFor(0)
  drawing.drawConnectors(
    pose.landmarks2d as Landmark2D[],
    POSE_CONNECTIONS.map(({ start, end }) => ({ end, start })),
    {
...
function drawHands(drawing: DrawingUtils, hands: HandState[]) {
  hands.forEach((hand) => {
    ...
    drawing.drawConnectors(
      hand.landmarks2d as Landmark2D[],
      HAND_CONNECTIONS.map(({ start, end }) => ({ end, start })),
      {
        color: colors.connector,
        lineWidth: OVERLAY_STYLES.connectorLineWidth,
      },
...
export function drawOverlay(
  ...
  if (state.face && showFace) drawFace(drawing, state.face)

  if (state.pose && showPose) drawPose(drawing, state.pose)

  if (state.hands?.length && showHands) drawHands(drawing, state.hands)
}
