  ...
  for (let row = 0; row < size; row++) {
    for (let column = 0; column <= row; column++) {
      ...
      if (row === column) {
        ...
        const diag = lower[column][column] || 1e-12
        lower[row][column] = Number.isFinite(value) ? value / diag : 0
      }
    }
  }
  return lower
...
export function solvePositiveDefinite(
  ...
  for (let row = 0; row < size; row++) {
    for (let output = 0; output < outputCount; output++) {
      ...
      const diag = lower[row][row] || 1e-12
      intermediate[row][output] = Number.isFinite(value) ? value / diag : 0
    }
  }
  ...
  for (let row = size - 1; row >= 0; row--) {
    for (let output = 0; output < outputCount; output++) {
      ...
      const diag = lower[row][row] || 1e-12
      solution[row][output] = Number.isFinite(value) ? value / diag : 0
    }
  }
  return solution
}
