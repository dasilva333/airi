...
import type { ThreeRendererMemorySnapshot, VrmSceneSnapshotInput, VrmSceneSummarySnapshot } from './types'
...
function maybeGetJsHeapUsedBytes() {
  ...
  return typeof heapUsed === 'number' && Number.isFinite(heapUsed) ? heapUsed : undefined
}
...
export function createVrmSceneSummarySnapshot(
  input?: VrmSceneSnapshotInput | VRM,
  maybeMixer?: AnimationMixer,
): VrmSceneSummarySnapshot {
  const params: VrmSceneSnapshotInput = input && 'scene' in input ? { mixer: maybeMixer, vrm: input } : (input ?? {})

  const activeVrm = params.vrm
  if (!activeVrm) {
    return {
      animationActionCount: params.mixer
        ? ((params.mixer as AnimationMixer & { _actions?: unknown[] })._actions?.length ?? 0)
        : 0,
      materialCount: 0,
      meshCount: 0,
      sceneChildCount: 0,
  ...
  activeVrm.scene.traverse((child) => {
    ...
    if (child instanceof SkinnedMesh) skinnedMeshCount += 1
