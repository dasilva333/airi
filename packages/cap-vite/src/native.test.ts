...
import {
  hasCapacitorTargetArg,
  parseCapacitorPlatform,
  pickServerUrl,
  resolveCapRunArgs,
  shouldRestartForNativeChange,
} from './native'
...
describe('pickServerUrl', () => {
  ...
  it('throws when vite did not expose any reachable url', () => {
    ...
    ).toThrow('Vite did not expose a reachable dev server URL.')
  })
})
...
describe('resolveCapRunArgs', () => {
  it('keeps an explicit --target argument untouched', () => {
    expect(
      resolveCapRunArgs(['ios', '--target', 'iPhone 16 Pro', '--scheme', 'AIRI'], {
  ...
  it('injects --target from CAPACITOR_DEVICE_ID when it is missing', () => {
    expect(resolveCapRunArgs(['android', '--flavor', 'release'], { CAPACITOR_DEVICE_ID: 'emulator-5554' })).toEqual([
      'android',
      '--target',
      'emulator-5554',
      '--flavor',
  ...
  it('supports the --target=value form when checking existing args', () => {
    ...
    expect(
      resolveCapRunArgs(['android', '--target=emulator-5554', '--flavor', 'release'], {
        CAPACITOR_DEVICE_ID: 'ignored-device',
      }),
...
describe('shouldRestartForNativeChange', () => {
  it('restarts for native source files inside the selected platform directory', () => {
    ...
    expect(
      shouldRestartForNativeChange('/repo/app/android/app/src/main/AndroidManifest.xml', 'android', '/repo/app'),
    ).toBe(true)
  })
  ...
  it('ignores web-side files and generated native output', () => {
    ...
    expect(shouldRestartForNativeChange('/repo/app/android/build/generated/file.kt', 'android', '/repo/app')).toBe(
      false,
    )
  })
