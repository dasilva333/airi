...
const emits = defineEmits<{
  ...
  (
    e: 'animationsDiscovered',
    value: { animations: { name: string; duration: number }[]; skins: { name: string }[] },
  ): void
...
function onCanvasMouseMove(event: MouseEvent) {
  if (!canvas.value || !skeleton || props.interactionMode !== 'tactile') return
  ...
  for (const area of model0HitAreas) {
    ...
    if (dist < radius) {
      ...
      if (hoveredArea !== area.name) {
        ...
        emits('hitAreaHover', { hovered: true, name: area.name, x: cssX, y: cssY })
      }
      break
    }
  }
...
function onCanvasClick(event: MouseEvent) {
  ...
  console.log(
    `[Spine] Canvas clicked at client: (${clickX}, ${clickY}), real: (${realClickX.toFixed(2)}, ${realClickY.toFixed(2)})`,
  )
  ...
  for (const area of model0HitAreas) {
    ...
    if (dist < radius) {
      ...
      console.log(
        `[Spine Audio] Looking up motionName="${motionName}", found=${!!motionConfig}, length=${motionConfig?.length ?? 0}`,
      )
      ...
      if (motionConfig && motionConfig.length > 0) {
        ...
        if (animationState) {
          ...
          if (entry) {
            entry.listener = {
              complete: () => {
                if (animationState) animationState.setEmptyAnimation(trackIndex, 0.2)
              },
            }
          }
        }
        ...
        console.log(
          `[Spine Audio] loadedBlobUrls exists=${!!loadedBlobUrls}, hasSoundKey=${!!(loadedBlobUrls && loadedBlobUrls[config.sound])}`,
        )
        if (loadedBlobUrls) {
        ...
        if (isStage && config.sound && loadedBlobUrls && loadedBlobUrls[config.sound]) {
          ...
          console.warn(
            `[Spine Audio] Audio skipped. isStage=${isStage}, sound="${config.sound}", blobUrlFound=${!!(loadedBlobUrls && loadedBlobUrls[config.sound])}`,
          )
        }
      }
...
async function loadModel() {
  ...
  try {
    ...
    if (isLocalBlob || modelSrc.value.endsWith('.zip')) {
      ...
      const selectedVariant = loaded.variants.find((v) => v.name === currentVariant.value) ?? loaded.variants[0]
      if (selectedVariant && currentVariant.value !== selectedVariant.name) currentVariant.value = selectedVariant.name
    ...
    await new Promise<void>((resolve, reject) => {
      const app: SpineCanvasApp = {
            ...
            const skeletonData =
              assetPaths.skeletonFormat === 'binary'
                ? new spine.SkeletonBinary(attachmentLoader).readSkeletonData(
                    am.require(assetPaths.skeletonPath) as Uint8Array,
                  )
                : new spine.SkeletonJson(attachmentLoader).readSkeletonData(
                    am.require(assetPaths.skeletonPath) as string,
                  )
                ...
                if (model0.motions) {
                  ...
                  for (const key in model0.motions) {
                    if (key !== 'idle' && !animations.find((a) => a.name === key)) {
                      animations.push({ duration: 0, name: key }) // virtual motion
                    }
                  }
                }
                if (model0.hit_areas) {
        ...
        loadAssets: (sc) => {
          ...
          if (blobUrls) patchAssetManagerForZipAssets(am, blobUrls, rawData!, assetPaths.texturePaths)

          if (assetPaths.skeletonFormat === 'binary') am.loadBinary(assetPaths.skeletonPath)
          else am.loadJson(assetPaths.skeletonPath)

          am.loadTextureAtlas(assetPaths.atlasPath)
        },
        render: (sc) => {
          if (!skeleton || !canvas.value) return
          ...
          if (!loggedBBoxThisLoad && canvas.value) {
            ...
            const rect = canvas.value.getBoundingClientRect()
            console.log('[Spine Debug] Stage/Canvas CSS BBox:', {
              height: rect.height,
              left: rect.left,
              top: rect.top,
              width: rect.width,
            })
            console.log('[Spine Debug] Canvas Internal Buffer:', {
              height: canvas.value.height,
              width: canvas.value.width,
            })
            console.log('[Spine Debug] Skeleton Origin:', {
            ...
            for (const area of model0HitAreas) {
              ...
              if (bone) {
                ...
                const boneCanvasY = canvas.value.height / 2 - bone.worldY
                console.log(
                  `[Spine Debug] Hit Area [${area.name}]: bone.worldX=${bone.worldX.toFixed(2)}, bone.worldY=${bone.worldY.toFixed(2)} | Calculated Center=(${boneCanvasX.toFixed(2)}, ${boneCanvasY.toFixed(2)})`,
                )
              }
            }
          }
...
function applyActiveAnimations(activeAnims: Record<string, boolean>) {
  ...
  if (!state || !availableAnimations.value) return
  ...
  availableAnimations.value.forEach((anim, index) => {
    ...
    if (isActive) {
      ...
      if (motionConfig && motionConfig.length > 0) {
        ...
        if (triggeredMotions.has(anim.name)) return
      ...
      // Regular skeleton animation — loop as usual
        ...
        if (!isPlaying && animationState) animationState.setAnimation(trackIndex, anim.name, true)
      }
    } else {
...
function setEmotion(emotion: Emotion, _intensity: number = 1): string | undefined {
  if (!animationManager) return undefined
...
watch(
  () => props.idleAnimationEnabled,
  () => {
    if (!animationManager || !skeleton || !animationState) return
    if (props.idleAnimationEnabled) applyCurrentAnimation()
    else animationState.setEmptyAnimation(0, props.defaultMixDuration)
  },
)

watch(
  () => props.defaultMixDuration,
  (mix) => {
    if (animationState) animationState.data.defaultMix = mix
  },
)
