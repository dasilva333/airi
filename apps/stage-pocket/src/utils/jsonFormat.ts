...
async function parseJSONFormat<S extends Schema, R extends SchemaOrString<S>>(
  content: string,
  options: { messages: Message[]; apiKey?: string; baseURL: string; model: string } & Partial<CommonRequestOptions>,
  schema?: S,
  erroredValue?: string,
  errorMessage?: string,
): Promise<R> {
  if (!schema) return content as unknown as R
 ...
 */
async function call<S extends Schema, R extends SchemaOrString<S>>(
  options: { messages: Message[]; apiKey?: string; baseURL: string; model: string } & Partial<CommonRequestOptions>,
  schema?: S,
): Promise<R> {
  if (schema != null) {
    options.messages.push(
      message.user(`Your response must follow the following schema:
${JSON.stringify(await toJsonSchema(schema))}
