  ...
  return {
    async bookmark(userId: string, characterId: string) {
      return await db.transaction(async (tx) => {
        ...
        if (existing) {
          await tx
            .delete(userCharacterSchema.characterBookmarks)
            .where(
              and(
                eq(userCharacterSchema.characterBookmarks.userId, userId),
                eq(userCharacterSchema.characterBookmarks.characterId, characterId),
    ...
    async create(data: {
      ...
      return await db.transaction(async (tx) => {
        ...
        if (data.capabilities?.length) {
          await tx
            .insert(schema.characterCapabilities)
            .values(data.capabilities.map((c) => ({ ...c, characterId: inserted.id })))
        }

        if (data.avatarModels?.length) {
          await tx.insert(schema.avatarModel).values(data.avatarModels.map((a) => ({ ...a, characterId: inserted.id })))
        }

        if (data.i18n?.length) {
          await tx.insert(schema.characterI18n).values(data.i18n.map((i) => ({ ...i, characterId: inserted.id })))
        }

        if (data.prompts?.length) {
          await tx.insert(schema.characterPrompts).values(data.prompts.map((p) => ({ ...p, characterId: inserted.id })))
        }
    ...
    async findByOwnerId(ownerId: string) {
      return await db.query.character.findMany({
        where: and(eq(schema.character.ownerId, ownerId), isNull(schema.character.deletedAt)),
        with: {
          bookmarks: true,
          capabilities: true,
          cover: true,
    ...
    async like(userId: string, characterId: string) {
      return await db.transaction(async (tx) => {
        ...
        if (existing) {
          await tx
            .delete(userCharacterSchema.characterLikes)
            .where(
              and(
                eq(userCharacterSchema.characterLikes.userId, userId),
                eq(userCharacterSchema.characterLikes.characterId, characterId),
