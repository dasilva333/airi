  ...
  return {
    execute: async (ctx, chatCtx, args) => {
      ...
      if (ctx.unreadEvents[channelId] && ctx.unreadEvents[channelId].length > 0) {
        ...
        return {
          result:
            'AIRI System: [INTERRUPT] Message sending ABORTED. New unread messages were detected from the user. Please [read_unread_messages] first to understand the new context.',
          shouldContinue: true,
          success: false,
        }
      }
