...
export async function readMessage(
  ...
  return await tracer.startActiveSpan('telegram.module.read_message', async (span) => {
    ...
    const lastNMessages = await tracer.startActiveSpan(
      'telegram.module.read_message.find_last_n_messages',
      async (span) => {
    ...
    logger
      .withField('number_of_tasks', unreadMessagesEmbeddingPromises.length)
      .log('Successfully embedded unread history messages')

    const unreadHistoryMessages = await Promise.all(
      state.unreadMessages[action.chatId].map((msg) => telegramMessageToOneLine(botId, msg)),
    )
    const unreadHistoryMessageOneliner = unreadHistoryMessages.join('\n')

    const existingKnownMessages = [
      ...unreadMessages.map((msg) => msg.message_id.toString()),
      ...lastNMessages.map((msg) => msg.platform_message_id),
    ]
    const relevantChatMessages = await tracer.startActiveSpan(
      'telegram.module.read_message.find_relevant_messages',
      async (span) => {
    ...
    logger
      .withField('number_of_relevant_chat_messages', relevantChatMessages.length)
      .log('Successfully composed relevant chat messages')
