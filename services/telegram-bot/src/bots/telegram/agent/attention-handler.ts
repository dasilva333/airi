...
export function createAttentionHandler(bot: BotContext, config: AttentionConfig) {
  ...
  const calculateNewResponseRate = () => {
    ...
    if (
      state.currentResponseRate <= config.responseRateMin &&
  ...
  const handler = {
    ...
    async shouldRespond(
      chatId: string,
      messages: Message[],
    ): Promise<{ shouldAct: boolean; reason: string; responseRate?: number }> {
      ...
      const reply = messages.some(
        (message) => message.reply_to_message?.from?.id.toString() === bot.bot.botInfo.id.toString(),
      )
