...
export async function findStickerByFileId(fileId: string) {
  const sticker = await useDrizzle().select().from(stickersTable).where(eq(stickersTable.file_id, fileId)).limit(1)
...
export async function findStickersByFileIds(fileIds: string[]) {
  const stickers = await useDrizzle().select().from(stickersTable).where(inArray(stickersTable.file_id, fileIds))

  return stickers
}

export async function recordSticker(
  ...
  await useDrizzle().insert(stickersTable).values({
    description,
    emoji,
    file_id: fileId,
...
export async function listRecentSentStickers() {
  return await useDrizzle().select().from(recentSentStickersTable).orderBy(desc(recentSentStickersTable.created_at))
}
