...
export async function recordMessage(botInfo: UserFromGetMe, message: Message) {
  ...
  await useDrizzle().insert(chatMessagesTable).values(values)
}
...
export async function findRelevantMessages(
  ...
  return await Promise.all(
    unreadHistoryMessagesEmbedding.map(async (embedding) => {
      ...
      logger
        .withField('number_of_relevant_messages', relevantMessages.length)
        .log('Successfully found relevant chat messages')
      ...
      //
      // [['123', '124']]
      const repliedMessageIDsSubset = relevantMessagesContext.map((msgs) =>
        msgs.filter((m) => m.is_reply).map((m) => m.reply_to_id),
      )
      // Convert from
      //
      // [['123', '124']]
      ...
      // Map into one liners
      const relevantMessageOneliner = relevantMessagesContext.map((msgs) =>
        msgs.map((m) => chatMessageToOneLine(botId, m, repliedMessagesSet.get(m.reply_to_id))).join('\n'),
      )
      logger
        .withField('number_of_relevant_messages', relevantMessages.length)
        .log('processed relevant chat messages with contextual messages')
