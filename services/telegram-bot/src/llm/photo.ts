...
export async function interpretPhotos(state: BotContext, msg: Message, photos: PhotoSize[]) {
  try {
    ...
    const photoResArray = await Promise.all(
      files.map((file) => fetch(`https://api.telegram.org/file/bot${state.bot.api.token}/${file.file_path}`)),
    )
    ...
    await Promise.all(
      photoBase64s.map(async (base64, index) => {
        const req = {
