...
export const stickersTable = pgTable(
  ...
  (table) => [
    index('stickers_description_vector_1536_index').using(
      'hnsw',
      table.description_vector_1536.op('vector_cosine_ops'),
    ),
    index('stickers_description_vector_1024_index').using(
      'hnsw',
      table.description_vector_1024.op('vector_cosine_ops'),
    ),
...
export const stickerPacksTable = pgTable(
  ...
  (table) => [uniqueIndex('sticker_packs_platform_platform_id_unique_index').on(table.platform, table.platform_id)],
)
...
export const joinedChatsTable = pgTable(
  'joined_chats',
  () => {
    return {
...
export const memoryFragmentsTable = pgTable(
  'memory_fragments',
  {
    ...
    last_accessed: bigint({ mode: 'number' })
      .notNull()
      .default(0)
      .$defaultFn(() => Date.now()),
...
export const memoryTagsTable = pgTable(
  'memory_tags',
  {
    created_at: bigint({ mode: 'number' })
      .notNull()
    ...
    memory_id: uuid()
      .notNull()
      .references(() => memoryFragmentsTable.id, { onDelete: 'cascade' }),
    tag: text().notNull(),
  },
  (table) => [index('memory_tags_memory_id_index').on(table.memory_id), index('memory_tags_tag_index').on(table.tag)],
)

// Episodic Memory (specific events)
export const memoryEpisodicTable = pgTable(
  'memory_episodic',
  {
    created_at: bigint({ mode: 'number' })
      .notNull()
...
// Goals table
export const memoryLongTermGoalsTable = pgTable(
...
export const memoryShortTermIdeas = pgTable(
  ...
  (table) => [
    ...
    index('memory_short_term_ideas_content_vector_1536_index').using(
      'hnsw',
      table.content_vector_1536.op('vector_cosine_ops'),
    ),
    index('memory_short_term_ideas_content_vector_1024_index').using(
      'hnsw',
      table.content_vector_1024.op('vector_cosine_ops'),
    ),
    index('memory_short_term_ideas_content_vector_768_index').using(
      'hnsw',
      table.content_vector_768.op('vector_cosine_ops'),
    ),
  ],
)
