...
const botAuthValues = ['mojang', 'microsoft', 'offline'] as const satisfies ReadonlyArray<
  NonNullable<BotOptions['auth']>
>
...
export function initEnv(): void {
  ...
  if (!parsedConfig.success) {
    logger
      .withFields({ issues: parsedConfig.error.issues })
      .error(`Invalid environment configuration: ${formatConfigValidationErrors(parsedConfig.error)}`)
