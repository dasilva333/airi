...
function createBotStub(
  options: { position?: Vec3; blocks?: Map<string, { name: string }>; entities?: Record<number, any> } = {},
) {
    ...
    blockAt: (queryPos: Vec3) => {
      ...
      if (found)
        return {
          ...
          diggable: true,
          name: found.name,
          position: new Vec3(Math.floor(queryPos.x), Math.floor(queryPos.y), Math.floor(queryPos.z)),
        }
      return {
        boundingBox: 'empty',
        diggable: false,
