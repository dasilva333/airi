...
function cloneCard(card: PatternCard): PatternCard {
  ...
  if (cloned.pitfalls) Object.freeze(cloned.pitfalls)
...
function scoreCard(card: PatternCard, tokens: string[]): number {
  ...
  for (const token of tokens) {
    ...
    if (titleTokens.includes(token) || intentTokens.includes(token)) score += 1
    if (whenToUseTokens.includes(token) || stepsTokens.includes(token)) score += 1
  }
  return score
}
