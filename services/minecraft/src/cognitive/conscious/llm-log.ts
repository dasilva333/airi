export type LlmLogEntryKind = 'turn_input' | 'llm_attempt' | 'repl_result' | 'repl_error' | 'scheduler' | 'feedback'
...
class LlmLogQuery {
    ...
    return this.clone({
      predicates: [
        ...
        (entry) => {
          ...
          if (sourceId !== undefined) return entry.sourceId === sourceId
          return true
        },
      ],
    })
  ...
  public list(): LlmLogEntry[] {
    ...
    if (this.sorter) result = [...result].sort(this.sorter)
    if (this.sliceLatest !== undefined) result = result.slice(0, this.sliceLatest)
