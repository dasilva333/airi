...
function inferDamageSource(ctx: {
  bot: { entity?: any; entities?: Record<string, any> }
  ...
  if (velocityY !== null && velocityY < -0.2 && onGround === false) return { cause: 'gravity' }
  ...
  for (const candidate of entities) {
    if (!candidate || ctx.isSelf(candidate)) continue

    const distance = ctx.distanceTo(candidate)
    if (distance === null || distance > ctx.maxDistance) continue
...
function inferCauseFromName(name: string): DamageSourceCause {
  ...
  if (name.includes('tnt') || name.includes('creeper') || name.includes('explosion')) return 'explosion'
  if (name.includes('arrow') || name.includes('trident') || name.includes('snowball')) return 'projectile'
  return 'unknown'
}
