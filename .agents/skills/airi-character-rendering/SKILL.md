...
- **Three.js VRM Decoupling from Vue DOM Remounts**: Calling `VRMUtils.deepDispose(vrm.value.scene)` on transient unmount (e.g. during Vue HMR, CSS updates, or temporary component re-renders) destroys WebGL buffers and forces an expensive re-download and re-parse. To prevent reload flickering, `modelStore` caches `activeVrmGroup` and `activeVrmInfo`, and `VRMModel.vue` distinguishes between shallow cleanup (`componentCleanUp(deepClean = false)`, which merely detaches the group from the scene and pauses render hooks/mixers) and deep disposal (`componentCleanUp(deepClean = true)`, executed only when switching to a different model identity or explicitly clearing the model).
