...
| **Spine Character** | `packages/stage-ui-spine/src/components/scenes/Spine.vue` | `packages/stage-ui-spine/src/components/scenes/spine/Model.vue` | Spec: [`docs/design-spine-model.md`](./design-spine-model.md) |
...
- **Manual Tool Call Indexing**: When manually constructing a tool call object inside the orchestrator pipeline (e.g., for bridged tool executions generated by parsing token markers), you **MUST** include the `"index": 0` field in the `tool_calls` object. Strict OpenAI/DeepSeek API gateways (like Console Go / OpenCode) enforce validation schemas on tool chunk structures; omitting the `index` property results in an immediate `400 Bad Request: Upstream request failed` error, even if the payload's structure is otherwise correct.
