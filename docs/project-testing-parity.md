...
|---|:---:|:---:|---|
| [`packages/stage-ui`](../packages/stage-ui) | 69 | 685 | Chat, Pacing, WebGPU Workers, BYOS Sync, Providers, Live2D, Memory, Artistry, Proactivity, MCP, Cloudflare OAuth, Gemini Live Bidi |
...
| **Monorepo Vitest Baseline** | **101 Suites** | **987 Tests** | **Automated Zero-Failure Headless Test Baseline** |

*(Note: 4 additional test files across `@proj-airi/stage-ui` and `@proj-airi/live2d-runtime` contain 8 tests conditional on external models or live API keys, yielding 105 total test files discovered).*
