...
export default defineConfig({
  content: {
    filesystem: ['.vitepress/**/*.{js,ts,vue}', 'content/**/*.md'],
  },
  preflights: [
    {
      getCSS: () => {
...
code,kbd,samp,pre {
  ...
  rules: [[/^bg-gradient-radial-\[(.+)\]$/, ([, d]) => ({ 'background-image': `radial-gradient(${d})` })]],
  safelist: [
    '-ml-8',
    'top-0',
    'hidden',
  ...
  theme: {
    ...
    animation: {
      ...
      keyframes: {
        ...
        swipeOut:
          '{from{transform:translateX(var(--reka-toast-swipe-end-x))}to{transform:translateX(calc(100% + var(--viewport-padding)))}}',
