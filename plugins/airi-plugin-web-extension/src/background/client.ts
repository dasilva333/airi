...
function sendContextUpdate(
  state: ClientState,
  update: Omit<ContextUpdate, 'id' | 'contextId'> & Partial<Pick<ContextUpdate, 'id' | 'contextId'>>,
) {
...
function sendSparkNotify(
  state: ClientState,
  data: { headline: string; note?: string; payload?: Record<string, unknown> },
) {
  if (!state.client || !state.connected) return
...
export function handlePageContext(state: ClientState, settings: ExtensionSettings, payload: PageContextPayload) {
  ...
  if (!settings.enabled || !settings.sendPageContext) return
...
export function handleVideoContext(
  ...
  if (!settings.enabled || !settings.sendVideoContext) return

  const headline = payload.title ? `User is watching: ${payload.title}` : 'User is watching a video'
...
export function handleSubtitle(state: ClientState, settings: ExtensionSettings, payload: SubtitlePayload) {
  ...
  if (!settings.enabled || !settings.sendSubtitles) return
