...
function collectFromExportsValue(value, out, source) {
  ...
  if (Array.isArray(value)) {
    for (const item of value) collectFromExportsValue(item, out, source)
    return
  }
  if (value && typeof value === 'object') {
    for (const [k, v] of Object.entries(value)) collectFromExportsValue(v, out, `${source}.${k}`)
  }
}
...
function collectEntryCandidates(pkg) {
  ...
  const pushIfString = (source, value) => {
    if (typeof value === 'string' && value.trim()) out.push({ raw: value.trim(), source })
  }
  ...
  if (typeof pkg.bin === 'string') {
    ...
    for (const [k, v] of Object.entries(pkg.bin)) pushIfString(`bin.${k}`, v)
  }

  if (typeof pkg.exports === 'string') {
    pushIfString('exports', pkg.exports)
  } else if (pkg.exports && typeof pkg.exports === 'object') {
    for (const [k, v] of Object.entries(pkg.exports)) collectFromExportsValue(v, out, `exports.${k}`)
  }
...
function resolveParentDir(pkgDir, rawTarget) {
  ...
  if (!target || /^(https?:|npm:|node:|data:)/.test(target)) return null
  ...
  let parentDir = abs
  if (ext) parentDir = path.dirname(abs)
  else if (logicalTarget.endsWith('/')) parentDir = abs
  else if (fs.existsSync(abs) && fs.statSync(abs).isFile()) parentDir = path.dirname(abs)
...
function printTable(rows, columns) {
  const widths = {}
  for (const col of columns) widths[col.key] = col.title.length
...
function main() {
  ...
  for (const rec of deduped) {
    ...
    if (!sccCache.has(rec.parentDir)) sccCache.set(rec.parentDir, runScc(rec.parentDir))
